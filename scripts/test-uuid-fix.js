...
/**
 * Script de test pour valider la correction UUID
 * Teste que l'application utilise correctement les UUIDs au lieu des IDs numériques
 */

const BASE_URL = 'http://localhost:3000';

async function testUUIDFix() {
  console.log('🧪 Test de la correction UUID...\n');
  ...
  try {
    ...
    // 3. Test des messages avec UUID
    console.log('\n3️⃣ Test des messages avec UUID...');
    const testChat = chatsWithUUIDs[0];
    console.log(`   Chat test: "${testChat.title}" (UUID: ${testChat.uuid})`);
    ...
    if (!messagesResponse.ok) {
      throw new Error(`Échec de récupération des messages: ${messagesResponse.status}`);
    }

    const messagesData = await messagesResponse.json();
    console.log(`✅ ${messagesData.length} messages récupérés avec UUID`);

    // 4. Test de création d'un nouveau chat
    console.log('\n4️⃣ Test de création d\'un nouveau chat...');
    const newChatResponse = await fetch(`${BASE_URL}/api/chat`, {
      ...
      body: JSON.stringify({
        title: 'Test UUID Fix',
        messages: {
          messages: [{ content: 'Test UUID Fix', is_user: true }],
          context: JSON.stringify({
            conversation_history: [{
              content: 'Test UUID Fix',
              is_user: true,
    ...
    const newChat = finalChatsData.items.find(chat => chat.title === 'Test UUID Fix');
    ...
    console.log('\n🎉 Tous les tests UUID sont passés avec succès !');
    console.log('\n📋 Résumé:');
